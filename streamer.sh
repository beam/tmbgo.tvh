...
${data}ffmpeg -fflags +genpts -i ${channel} -vcodec copy -acodec copy -f mpegts -mpegts_service_type digital_tv -metadata service_provider=${provider} -metadata service_name="${channel_name}" pipe:1
